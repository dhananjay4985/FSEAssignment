...
public class LibraryDAOImpl implements LibraryDAO {
	...
	public void searchBook(long bookId) {
		try {
			connection = dataSource.getConnection();			
			pstmtBook = connection.prepareStatement(Constants.SEARCH_BOOK);		
	...
	public void searchSubject(long subjectId) {		
		try {
			connection = dataSource.getConnection();			
			pstmtSubject = connection.prepareStatement(Constants.SEARCH_SUBJECT);		
	...
	public void displayBookByTitle() {
		try {
			connection = dataSource.getConnection();			
			pstmtBook = connection.prepareStatement(Constants.SELECT_ALL_BOOK);			
			resultSet = pstmtBook.executeQuery();
			...
			if (resultSet.isBeforeFirst()) {				
				...
				bookList.forEach(obj -> System.out.println("Book Id: "+obj.getBookId()+",Price: "+obj.getPrice()+",Title: "+obj.getTitle()+",Volume: "+obj.getVolume()+",PublishDate: "+obj.getPublishDate()));	
			}else {
	...
	@Override
	public void displayBookByPublishDate() {
		try {
			connection = dataSource.getConnection();			
			pstmtBook = connection.prepareStatement(Constants.SELECT_ALL_BOOK);			
			resultSet = pstmtBook.executeQuery();
	...
	@Override
	public void displaySubjectByTitle() {

		try {
			connection = dataSource.getConnection();			
			pstmtSubject = connection.prepareStatement(Constants.SELECT_ALL_SUBJECT);			
			resultSet = pstmtSubject.executeQuery();
			...
			if (resultSet.isBeforeFirst()) {				
				while (resultSet.next()) {					
					...
					subject = new Subject(resultSet.getInt("subjectid"),resultSet.getString("subjectTitle"),resultSet.getInt("durationinhours"), bookSet);
					subjectList.add(subject);
				}
				subjectList = subjectList.stream().sorted(Comparator.comparing(Subject::getSubjectTitle)).collect(Collectors.toList());
				subjectList.forEach(obj -> System.out.println("Subject Id: "+obj.getSubjectId()+",Title: "+obj.getSubjectTitle()+",Duration: "+obj.getDurationInHours()+"Hrs"+",Book:" +obj.getBookSet().toString()));
